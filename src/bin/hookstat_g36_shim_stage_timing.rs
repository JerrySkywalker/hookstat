...
mod windows_timing {
    ...
    use super::hook_shim::{
        ...
        OriginalHandlerBudget, capsule_file_name, run_capsule_for_qualification_timed,
        write_key_for_test,
    };
    ...
    fn statistics(mut samples: Vec<u64>) -> Statistics {
        ...
        let value = |percent: usize| {
            samples[(samples.len() * percent).div_ceil(100) - 1] as f64 / 1_000_000.0
        };
        Statistics {
            samples: samples.len(),
            p50_ms: value(50),
    ...
    pub(super) fn main() {
        ...
        if !breakaway_worker {
            ...
            match status {
                ...
                Ok(_) | Err(_) => {
                    eprintln!(
                        "hookstat-g36-shim-stage-timing could not establish breakaway admission"
                    );
                    std::process::exit(1);
                }
            }
        ...
        for _ in 0..samples {
            ...
            let (outcome, stages) =
                run_capsule_for_qualification_timed(&loaded, &state_root).unwrap();
            assert_eq!(
                outcome.exit_code, 0,
...
#[cfg(not(windows))]
fn main() {
    eprintln!("hookstat-g36-shim-stage-timing is Windows-only");
    std::process::exit(2);
}
