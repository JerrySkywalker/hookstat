...
mod windows_oracle {
    ...
    use super::hook_shim::{
        CapsuleStore, ExecutionPlan, HandlerCapsule, InstrumentationEnvelope,
        OriginalHandlerBudget, capsule_file_name, write_key_for_test,
    };
    use super::ipc_client::{
        Completion, CooperativeProducer, ExitClassification, LifecycleFrame, Listener,
    ...
    fn statistics(mut values: Vec<u64>) -> Statistics {
        ...
        let value = |percent: usize| {
            values[(values.len() * percent).div_ceil(100) - 1] as f64 / 1_000_000.0
        };
        Statistics {
            samples: values.len(),
            p50_ms: value(50),
    ...
    fn run(output: PathBuf, shipping: PathBuf, instrumented: PathBuf, samples: usize) {
        ...
        let endpoint =
            LocalEndpoint::from_state_root(&oracle_root).expect("create oracle endpoint");
        ...
        let (shipping_startup, instrumented_startup) =
            startup_pairs(&shipping, &instrumented, samples);
