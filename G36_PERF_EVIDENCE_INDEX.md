...
| `runs/HS-G36-WARM-BUDGET-RECALIBRATION-LANDING-004/g36-host-admitted-qualification-540f0be-001-startup-comparability-001.json` | `INVALIDATED_BY_BUILD_PROFILE` | The first prospective recalibrated-contract run completed the exact pre/comparison/post comparator before any product sample. Both controls pass, but the complete-envelope shipping-minus-instrumented p99 correction is `14.1492 ms`, above the fixed `2.0 ms` stop. | The comparator retains p50 values within about `0.5 ms` while shipping alone contains large p99 tails. Under the predefined rule, the instrumented oracle cannot represent shipping in this attempt. It proves neither product PASS nor product FAIL; no product window ran. |
| `runs/HS-G36-WARM-BUDGET-RECALIBRATION-LANDING-004/g36-host-admitted-qualification-660142c-001-startup-comparability-*.json` | `DIAGNOSTIC_ONLY` | Five exact-head startup-comparability attempts retain complete pre/comparison/post populations and `REJECTED_HOST_SUBSTRATE`. Each failed at least one predefined `20/25`-ms host control, so build correction and product qualification were not admitted. | Five host windows are retained as rejections, not product failures. No product sample ran in this session. |
